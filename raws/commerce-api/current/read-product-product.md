...
> 원문: https://apicenter.commerce.naver.com/docs/commerce-api/current/read-product-product
...
**specificProducts** 그룹상품 상품별 응답 (object)[]
...
**standardPurchaseOptions** 판매 옵션 정보 (object)[]
...
**customerBenefit** 그룹상품 고객 혜택 정보 응답 (object)
...
- 상품 등록/수정 시: NORMAL(일반 배송), TODAY(오늘출발), OPTION\_TODAY(옵션별 오늘출발), HOPE(희망일배송), TODAY\_ARRIVAL(당일배송(지금배달 관련 기능)), DAWN\_ARRIVAL(새벽배송(지금배달 관련 기능)), ARRIVAL\_GUARANTEE(N배송), SELLER\_GUARANTEE(N판매자배송), HOPE\_SELLER\_GUARANTEE(N희망일배송)
...
- 그룹상품 등록/수정 시: NORMAL(일반 배송), TODAY(오늘 출발), HOPE(희망일배송), ARRIVAL\_GUARANTEE (N배송), SELLER\_GUARANTEE(N판매자배송), HOPE\_SELLER\_GUARANTEE (N희망일배송)
...
```bash
curl -L 'https://api.commerce.naver.com/external/v2/standard-group-products/:groupProductNo' \
