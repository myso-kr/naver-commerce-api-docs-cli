...
> 원문: https://apicenter.commerce.naver.com/docs/commerce-api/current/update-origin-product-product
...
**BAD\_REQUEST 오류 응답 시, InvalidInputs 필드 정보가 없거나 InvalidInputs 필드만으로 정확한 판단이 어려울 수 있습니다. 이때는 message 내용을 활용한 오류 판단을 권장합니다.**
...
상품 공통 속성. 원상품에 속한 채널 상품은 모두 상품 공통 속성을 참고하게 됩니다.

**statusType**상품 판매 상태 코드 (string)required

상품 API에서 상품의 판매 상태를 나타내기 위해 사용하는 코드입니다.
상품 등록 시에는 SALE(판매 중)만 입력할 수 있으며, 상품 수정 시에는 SALE(판매 중), SUSPENSION(판매 중지)만 입력할 수 있습니다. 상품 등록 시에는 SUSPENSION(판매 중지)으로 입력하면 SALE(판매 중)로 등록됩니다. StockQuantity의 값이 0인 경우 상품 상태는 OUTOFSTOCK(품절)으로 저장됩니다.
품절 상태의 상품을 판매 중으로 변경하는 경우, StockQuantity(재고 수량)와 함께 statusType을 SALE(판매 중)로 입력해야 합니다.
...
상품 등록 시 필수입니다.
상품 수정 시 카탈로그 ID(modelId)를 입력한 경우 필수입니다.
표준형 옵션 카테고리 상품 수정 요청의 경우 CategoryId 변경 요청은 무시됩니다.(렌탈상품은 정상 처리됨)
...
**detailAttribute** 원상품 상세 속성 (object)required
...
**afterServiceInfo** A/S 정보 (object)required
...
- 옵션상품인 경우 옵션정보(optionInfo)에 입력된 값이 적용됩니다.
...
인증 유형이 방송통신기자재 적합인증/적합등록/잠정인증인 경우 필수. 'yyyy-MM-dd' 형식 입력.
...
네이버 상품 API에서 부가가치세의 타입을 나타내기 위해 사용하는 코드입니다. 미입력 시 과세 상품으로 등록됩니다.
...
SellerCommentUsable 값이 true일 때 입력합니다.

**sellerCommentUsable**판매자 특이 사항 사용 여부 (boolean)

**minorPurchasable**미성년자 구매 가능 여부 (boolean)required

성인 카테고리의 경우 불가능으로 입력해야 합니다.
렌탈의 경우 미성년자에게 상품 노출 여부를 입력합니다.
...
예외 카테고리 중 도서\_일반, 도서\_해외, 도서\_중고, 도서\_E북, 도서\_오디오북, 문화비\_소득공제에 해당하는 경우 입력할 수 있습니다.
미입력 시 false로 저장됩니다.
...
**customerBenefit** 상품 고객 혜택 정보 (object)
...
**smartstoreChannelProduct** 스마트스토어 채널상품 정보 구조체 (object)required
...
**windowChannelProduct** 쇼핑윈도 채널상품 정보 구조체 (object)
...
**rentalInfo** 렌탈 정보 (object)

렌탈 정보

**exclusiveRentalProductYn**렌탈 전용 상품 여부 (boolean)required

**retailPrice**일시불 소비자 판매가 (number)

렌탈전용상품여부N일경우 필수

**basisCreditScore**신용 평가 기준 점수 (integer<int32>)

**contractGuideInfo** 렌탈 계약 안내사항 정보 (object)

렌탈 계약 안내사항 정보

**guideContents** 계약 안내사항 정보 (object)[]

- Array [

**guideType**계약안내사항 구분 (소모품비, 철거비, 이전설치비, 분실/파손비, 기타 추가요금) (string)

**Possible values:** [`CONSUMABLE_FEE`, `WITHDRAWAL_FEE`, `TRANSFER_INSTALLATION_FEE`, `BREAKAGE_FEE`, `ADDITIONAL_FEE`]

**guideTypeNameInput**기타 추가요금일 경우 항목명 (string)

**feeValue**계약 안내사항 금액 (number)

**feeText**계약 안내사항 사용자 입력 텍스트 (string)

**conditionalFeeYn**조건에 따라 상이 여부 (boolean)

- ]

**onetimeFeeInfo** 렌탈 일회성요금 정보 (object)

렌탈 일회성요금 정보

**onetimeFeeUsable**일회성요금 설정여부 (boolean)

**onetimeFeePaymentType**일회성 요금 납입 시점 (일회성요금 설정 true일 경우) (string)

**Possible values:** [`DIRECT`, `AFTER_ONE_TERM`]

**onetimeFeeContents** 일회성요금 정보 (일회성요금 설정 true일 경우) (object)[]

- Array [

**feeType**일회성요금 구분 (등록비, 가입비, 설치비, 기타 추가요금) (string)

**Possible values:** [`REGISTRATION_FEE`, `ADDITIONAL_FEE`]

**feeValue**일회성 요금 금액 (number)

**feeTypeNameInput**기타 추가요금일 경우 항목명 (길이제한 체크) (string)

**exemptionYn**일회성요금 면제여부
- 미입력 시 false(면제설정 안함) 으로 저장됩니다. (boolean)

- ]

**rentalTermsPolicyId**렌탈 약관 정책 일련번호 (integer<int64>)required

**rentalPenaltyPolicyId**렌탈 위약금 정책 일련번호 (integer<int64>)required

**rentalContractConditions** 렌탈 계약조건 정보 (object)[]required

- Array [

**contractPeriod**계약기간 (integer<int32>)required

**commitmentPeriod**의무사용기간 (integer<int32>)required

**managementMethodType**관리유형 설정 타입
-관리유형 타입으로는 방문관리(VISIT), 직접관리(ONESELF) 중 선택하여 입력해주세요. 상품에 관리유형을 설정하지 않으시려면 전체 계약조건에 설정하지 않음(NONE)을 입력해주세요. (ALL)은 설정할 수 없으며, 미입력 시 NONE으로 설정됩니다. (string)

**Possible values:** [`ALL`, `NONE`, `VISIT`, `ONESELF`]

**selfManagementAdditionalServiceText**직접관리 부가서비스 제공 입력값 (관리유형>직접관리일 경우) (string)

**visitIntervals**월단위 방문주기 (관리유형>방문관리일 경우) (integer<int32>)

**replacementServiceType**교체서비스 설정 타입
-교체서비스 타입으로는 포함(INCLUDE), 미포함(NOT\_INCLUDE) 중 선택하여 입력해주세요. 상품에 교체서비스를 설정하지 않으시려면 전체 계약조건에 설정하지 않음(NONE)을 입력해주세요. 미입력 시 NONE으로 설정됩니다. (string)

**Possible values:** [`NONE`, `INCLUDE`, `NOT_INCLUDE`]

**replacementUnitName**교체품명 (string)

**replacementCount**교체횟수 (교체서비스 Y일 경우) (integer<int32>)

**advancedPaymentFeeType**선납금여부 설정 타입
-선납금여부 타입으로는 선납금결제(PAID), 선납금없음(NO\_SELECT) 중 선택하여 입력해주세요. 단, 최소 한개 이상의 계약조건에 선납금없음(NO\_SELECT)을 입력하셔야합니다. 상품에 선납금여부를 설정하지 않으시려면 전체 계약조건에 설정하지 않음(NONE)을 입력해주세요. 미입력 시 NONE으로 설정됩니다. (string)

**Possible values:** [`NONE`, `PAID`, `NO_SELECT`]

**advancedPaymentFeeValue**선납금 금액값 (number)

**ownershipTransferType**소유권이전 설정 타입 (string)required

**Possible values:** [`NO_TRANSFER`, `AUTO`]

**rentalMonthlyFee**월렌탈료(할인전) (number)required

**naverShoppingPriceYn**네이버쇼핑 가격비교 노출대상 여부 (boolean)

**sellerManagerCode**판매자관리코드 (string)

**usable**사용 여부
-미입력 시 사용 여부는 TRUE로 설정됩니다. (boolean)

- ]

**rentalContractBenefits** 렌탈 할인 정보 (object)[]

- Array [

**contractPeriodType**할인 계약기간 타입 (전체, 특정 기간) (string)

**Possible values:** [`ALL`, `CONDITIONAL`]

**contractPeriod**할인 계약기간 타입이 특정 기간일 경우 특정 선택된 계약기간 (integer<int32>)

**commitmentPeriodType**할인 의무사용기간 타입 (전체, 특정 기간) (string)

**Possible values:** [`ALL`, `CONDITIONAL`]

**commitmentPeriod**할인 의무사용기간 타입이 특정 기간일 경우 선택된 의무사용기간 (integer<int32>)

**managementMethodType**할인 관리유형 타입 (string)

**Possible values:** [`ALL`, `NONE`, `VISIT`, `ONESELF`]

**discountName**판매자지정 할인이름 (string)required

**promotionCode**프로모션코드 (string)

**discountTermType**할인대상 회차 (전체회차, 기간회차, 특정회차) (string)required

**Possible values:** [`ALL`, `PERIOD`, `CONDITIONAL`]

**exemptionYn**금액 면제 여부 (boolean)required

**benefitValue**혜택 값 (number)

**conditionalDiscountTerms**할인대상 회차에서 특정회차 선택했을 경우, 특정 회차 값들을 저장. (integer<int32>)[]

**benefitUnitType**혜택단위타입 (string)

**Possible values:** [`PERCENT`, `WON`, `YEN`, `COUNT`]

**startPeriod**할인대상 회차에서 기간회차 선택했을 경우, 기간회차 시작값. (integer<int32>)

**endPeriod**할인대상 회차에서 기간회차 선택했을 경우, 기간회차 종료값. (integer<int32>)

**startDate**적용 시작일시 (string<date-time>)

매 시각 00, 10, 20, 30, 40, 50분으로만 설정 가능합니다. 'yyyy-MM-dd'T'HH:mm[:ss][.SSS]XXX' 형식으로 입력합니다.

**endDate**적용 종료일시 (string<date-time>)

매 시각 09, 19, 29, 39, 49, 59분으로만 설정 가능합니다. 'yyyy-MM-dd'T'HH:mm[:ss][.SSS]XXX' 형식으로 입력합니다.

**regOrder**등록 수정시 순서 (integer<int32>)
...
**scheduledPointPolicy** 렌탈 정기결제 포인트 정책 (object)

렌탈 정기결제 포인트 혜택

**value**렌탈 정기결제 포인트 값(%) (number)required

**Possible values:** `>= 0.1` and `<= 30`

**startDate**렌탈 정기결제 포인트 시작일 (string<date>)

'yyyy-MM-dd' 형식 입력.

**endDate**렌탈 정기결제 포인트 종료일 (string<date>)

'yyyy-MM-dd' 형식 입력.

**rentalGiftPointPolicy** 렌탈 감사 포인트 정책 (object)

렌탈 감사 포인트 정책

**value**렌탈 감사 포인트 값 (number)required

**Possible values:** `>= 1` and `<= 1000000`

**startDate**렌탈 감사 포인트 시작일 (string<date-time>)

매 시각 00, 10, 20, 30, 40, 50분으로만 설정 가능합니다. 'yyyy-MM-dd'T'HH:mm[:ss][.SSS]XXX' 형식으로 입력합니다.

**endDate**렌탈 감사 포인트 종료일 (string<date-time>)

매 시각 09, 19, 29, 39, 49, 59분으로만 설정 가능합니다. 'yyyy-MM-dd'T'HH:mm[:ss][.SSS]XXX' 형식으로 입력합니다.

**rentalReservedPointPolicy** 렌탈 예약 포인트 정책 (object)

렌탈 예약 포인트 정책

**value**렌탈 예약 포인트 값 (number)required
...
**startDate**렌탈 예약 포인트 시작일 (string<date-time>)

매 시각 00, 10, 20, 30, 40, 50분으로만 설정 가능합니다. 'yyyy-MM-dd'T'HH:mm[:ss][.SSS]XXX' 형식으로 입력합니다.

**endDate**렌탈 예약 포인트 종료일 (string<date-time>)
...
**smartstoreChannelProduct** 스마트스토어 채널 상품 (object)
...
**windowChannelProduct** 윈도 채널 상품 (object)
...
**channelProductDisplayStatusType**전시 상태 코드(윈도 채널 읽기 전용) (string)
...
```json
{
  ...
  "originProduct": {
    ...
    "detailAttribute": {
      ...
      "rentalInfo": {
        "exclusiveRentalProductYn": true,
        "retailPrice": 0,
        "basisCreditScore": 0,
        "contractGuideInfo": {
          "guideContents": [
            {
              "guideType": "CONSUMABLE_FEE",
              "guideTypeNameInput": "string",
              "feeValue": 0,
              "feeText": "string",
              "conditionalFeeYn": true
            }
          ]
        },
        "onetimeFeeInfo": {
          "onetimeFeeUsable": true,
          "onetimeFeePaymentType": "DIRECT",
          "onetimeFeeContents": [
            {
              "feeType": "REGISTRATION_FEE",
              "feeValue": 0,
              "feeTypeNameInput": "string",
              "exemptionYn": true
            }
          ]
        },
        "rentalTermsPolicyId": 0,
        "rentalPenaltyPolicyId": 0,
        "rentalContractConditions": [
          {
            "contractPeriod": 0,
            "commitmentPeriod": 0,
            "managementMethodType": "ALL",
            "selfManagementAdditionalServiceText": "string",
            "visitIntervals": 0,
            "replacementServiceType": "NONE",
            "replacementUnitName": "string",
            "replacementCount": 0,
            "advancedPaymentFeeType": "NONE",
            "advancedPaymentFeeValue": 0,
            "ownershipTransferType": "NO_TRANSFER",
            "rentalMonthlyFee": 0,
            "naverShoppingPriceYn": true,
            "sellerManagerCode": "string",
            "usable": true
          }
        ],
        "rentalContractBenefits": [
          {
            "contractPeriodType": "ALL",
            "contractPeriod": 0,
            "commitmentPeriodType": "ALL",
            "commitmentPeriod": 0,
            "managementMethodType": "ALL",
            "discountName": "string",
            "promotionCode": "string",
            "discountTermType": "ALL",
            "exemptionYn": true,
            "benefitValue": 0,
            "conditionalDiscountTerms": [
              0
            ],
    ...
    "customerBenefit": {
      ...
      "scheduledPointPolicy": {
        "value": 0,
        "startDate": "2024-07-29",
        "endDate": "2024-07-29"
      },
      "rentalGiftPointPolicy": {
        "value": 0,
        "startDate": "2024-07-29T15:51:28.071Z",
        "endDate": "2024-07-29T15:51:28.071Z"
      },
      "rentalReservedPointPolicy": {
        "value": 0,
        "startDate": "2024-07-29T15:51:28.071Z",
        "endDate": "2024-07-29T15:51:28.071Z"
      },
...
```bash
curl -L -X PUT 'https://api.commerce.naver.com/external/v2/products/origin-products/:originProductNo' \
