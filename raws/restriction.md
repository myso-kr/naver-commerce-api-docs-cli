...
> 원문: https://apicenter.commerce.naver.com/docs/restriction

커머스API의 각 기능별 설명 영역에서는 다음과 같이 '제약 사항'에 대해서 안내합니다.

### 웹 서버 인증서(TLS) 버전 제약[​](#웹-서버-인증서tls-버전-제약 "Direct link to 웹 서버 인증서(TLS) 버전 제약")

커머스API는 웹 서버 인증서(TLS) 버전 1.2 이상에서만 정상 동작합니다.

### 권한 그룹[​](#권한-그룹 "Direct link to 권한 그룹")
...
### 요청량 제한[​](#요청량-제한 "Direct link to 요청량 제한")
...
#### Rate Limit: 초당 최대 동시 요청 수 제한[​](#rate-limit-초당-최대-동시-요청-수-제한 "Direct link to Rate Limit: 초당 최대 동시 요청 수 제한")
...
#### Quota Limit: (단위) 시간당 판매자 리소스 요청 수 제한[​](#quota-limit-단위-시간당-판매자-리소스-요청-수-제한 "Direct link to Quota Limit: (단위) 시간당 판매자 리소스 요청 수 제한")
