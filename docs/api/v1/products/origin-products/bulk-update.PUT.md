---
doc-id: "v1-products-origin-products-bulk-update-put"
title: "상품 벌크 업데이트"
description: "상품의 특정 항목을 일괄 수정합니다."
type: api-endpoint
method: PUT
...
```
> **PUT** `https://api.commerce.naver.com/external/v1/products/origin-products/bulk-update`
```
...
- IMMEDIATE_DISCOUNT(기본 할인), SALE_PRICE(판매가), SALE_PERIOD(판매 기간), DELIVERY(배송 정보), DELIVERY_ATTRIBUTE(배송 속성), DELIVERY_HOPE(희망일배송), PURCHASE_BENEFIT(구매/리뷰 혜택), PURCHASE_QUANTITY_LIMIT(구매 수량 제한)
...
- NORMAL(일반 배송), TODAY(오늘출발), HOPE(희망일배송)-productBulkUpdateType이 'DELIVERY_HOPE'인 경우에만 설정 가능
...
**PUT** `/v1/products/origin-products/bulk-update` — 응답
성공
