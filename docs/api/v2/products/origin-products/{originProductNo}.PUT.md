---
doc-id: "v2-products-origin-products-originProductNo-put"
title: "(v2) 원상품 수정"
description: "BAD_REQUEST 오류 응답 시, InvalidInputs 필드 정보가 없거나 InvalidInputs 필드만으로 정확한 판단이 어려울 수 있습니다. 이때는 message 내용을 활용한 오류 판단을 권장합니다."
type: api-endpoint
method: PUT
...
```
> **PUT** `https://api.commerce.naver.com/external/v2/products/origin-products/{originProductNo}`
```
...
예외 카테고리 중 도서_일반, 도서_해외, 도서_중고, 도서_E북, 도서_오디오북, 문화비_소득공제에 해당하는 경우 입력할 수 있습니다.
미입력 시 false로 저장됩니다.
...
**PUT** `/v2/products/origin-products/{originProductNo}` — 응답
성공
...
-교체서비스 타입으로는 포함(INCLUDE), 미포함(NOT_INCLUDE) 중 선택하여 입력해주세요. 상품에 교체서비스를 설정하지 않으시려면 전체 계약조건에 설정하지 않음(NONE)을 입력해주세요. 미입력 시 NONE으로 설정됩니다. (string)
...
-선납금여부 타입으로는 선납금결제(PAID), 선납금없음(NO_SELECT) 중 선택하여 입력해주세요. 단, 최소 한개 이상의 계약조건에 선납금없음(NO_SELECT)을 입력하셔야합니다. 상품에 선납금여부를 설정하지 않으시려면 전체 계약조건에 설정하지 않음(NONE)을 입력해주세요. 미입력 시 NONE으로 설정됩니다. (string)
